  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-[#FAF7F2]/95 backdrop-blur-md border-t border-[#DCD5C6] flex justify-around py-2.5 shadow-[0_-4px_16px_rgba(0,0,0,0.06)] z-30 px-2 pb-safe">
      ...
      <button
        ...
        onClick={onOpenCart}
        className={`flex flex-col items-center py-1 px-3 rounded-2xl text-[#8A7E6B] hover:text-[#1B4D3E] hover:bg-black/5 hover:-translate-y-0.5 relative transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] active:scale-90 cursor-pointer ${
          cartBump ? 'animate-pop-bump text-[#1B4D3E] font-bold bg-[#1B4D3E]/10' : ''
        }`}
      >
        <div className="relative">
          <ShoppingBasket className={`w-5 h-5 transition-transform duration-300 ${cartBump ? 'scale-110' : ''}`} />
          {cartCount > 0 && (
            <span

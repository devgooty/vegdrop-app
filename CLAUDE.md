...
- **The launch screen hands its wordmark to the login screen rather than cutting to it.** Both draw "VegDrop" in the same face, weight and tracking, so redrawing it at a new size and place reads as two screens that happen to share a logo. Instead the splash takes its lockup apart — plate dissolved, droplet drained — leaving the wordmark alone on white, publishes where it was standing (`src/lib/brandFlight.js`), and the login screen flies its own wordmark from there into place. Three things this depends on: the splash must not move the wordmark or the layout around it during that exit, since the position is measured once at the start of it; the flight measures `.si-hero-wordmark-text`, an inline-block wrapper, because the div around it is as wide as the hero and would give a size ratio of two different things; and the whole thing is opt-in per launch (`handoff`), because that exit ends on a bare wordmark and only the login screen has anywhere to put it. Deliberately **not** `document.startViewTransition` — see the reasoning at the top of `brandFlight.js`. Only the customer login has a wordmark in the DOM; the shopkeeper and delivery heroes have theirs painted into the artwork, so they keep the plain fade.

...
An account is pictured in one of two ways: one of the built-in avatars, or the first letter of the name. The second is a fallback, not a second option — an account that has chosen nothing and one carrying an avatar key this build does not recognise both land there.

**Uploading a photo was removed, and removed completely.** Gone: the `UserAvatar` collection, `GET /api/users/:id/avatar`, the `{ image }` branch of the write, the size cap and `config.avatar`, the jpeg/webp allow-list, `avatar.photoUpdatedAt`, `toAvatarJpeg` in `services/imageCapture.js`, the picker's Photo tab, and the client-side fetch keyed on that timestamp. `db/migrations.js` → `migrateRemovedAvatarPhotos()` drops the stored photographs on every boot.

**The bytes are deleted rather than left orphaned, and that is the point of the migration.** Nothing reads them any more, so leaving them would break nothing — they would simply be photographs of people's faces that their owners can no longer see, edit or delete, because every screen and endpoint that reached them is gone. A dropped feature must not turn its data into a permanent holding of personal images nobody can act on. `avatar.photoUpdatedAt` goes for the same reason `emailVerifiedAt` did: a leftover timestamp asserting "this account has a photo" is exactly what a later change reads again without checking whether anything still writes it.

**What the deletion took with it is worth knowing, because it all comes back with the feature** rather than being re-derived: user-supplied bytes rendered back into a page (which is why SVG was refused — it is a script container), a body limit an order of magnitude above every other route's, EXIF stripping performed in the browser where it cannot be relied on, and a second collection to keep in step with the user record. A drawn avatar has none of them.

**`PUT /api/users/:id/avatar` stays a single writer on its own path.** It now takes `{ preset, skinTone?, hair? }` and nothing else — `.strict()` refuses an `image`, so a client sending one is told 400 rather than having it silently stripped and being told 200. `.strict()` likewise still refuses `avatar` on `PATCH /api/users/:id`, so the picture keeps exactly one door.

**One removal has already bitten, and it is the reason to read `app.js` before touching this route.** `LARGE_BODY_PATHS` there names paths that are *deliberately left unparsed* so a route can mount its own bigger parser. The avatar path was one of them. Deleting the route's parser without removing that entry left `req.body` undefined, and every valid write failed validation as "expected object, received undefined". The entry is gone and the route takes the ordinary 100 KB parser; an exemption that outlives its reason is a raised ceiling nobody remembers granting.

**Three slugs travel on the profile read, and there is no companion fetch.** `User.avatar` carries `preset`, `skinTone` and `hair`, all cosmetic and all tiny — which matters because `middleware/auth.js` re-reads the whole `User` document on every authenticated request and the three role apps poll orders every five seconds. That hot path is exactly why the image was never on this document, and why nothing image-shaped should be put back on it. Same reasoning, at length, on `models/StallPhoto.js`.
...
**This is not local to the price sheet: every date anywhere is a market day.** `startOfMarketDay()` is the only correct way to say "since midnight", and an aggregation that buckets by date must pass `timezone` to `$dateToString` — `routes/developer.js` builds that string once as `MARKET_TZ` from the same config value, so the pipeline and the JS cannot disagree about where a day starts. Three separate clocks had got into one endpoint there: KPIs on the server's local midnight, chart buckets on UTC, and weekday labels off a UTC date string read with local `getDay()`. On the UTC host this runs on, that filed every order placed before 05:30 IST under the previous day, so "Today's Orders" read zero through the market's busiest hour. `new Date('2026-09-12').getDay()` is the specific trap in the label case: it parses as UTC midnight and then reads the weekday in the server's zone.
...
### Handover codes — the rider types both

Every order changes hands twice, and each handover is confirmed by a six-digit code the **rider types** and **somebody else is shown**:

| Stage | Shown to | Typed at | Route the rider calls |
|---|---|---|---|
| pickup, independent shop | the shop, from the moment it accepts | the counter | `POST /api/orders/:id/verify-pickup` |
| pickup, market | **each stall**, its own code | that stall's pitch | `POST /api/rider/orders/:id/collect` `{ stallId, code }` |
| delivery, any order | the customer, once it is out for delivery | the door | `POST /api/orders/:id/verify-delivery`, or `/api/rider/orders/:id/deliver` for a market order |

Holders read their own code from `GET /api/orders/:id/pickup-code`, `GET /api/stalls/orders/:id/pickup-code` and `GET /api/orders/:id/delivery-code`, each with a `/reissue` sibling. `services/handover.js` holds the logic; `components/HandoverCodeCard.jsx` shows a code and `HandoverCodeEntry.jsx` takes one.

**The rule everything else follows: whoever submits a code must never be able to read it.** The rider submits both, so no endpoint may put either code in front of a `delivery` session. It used to run the other way for pickup — the rider was shown `Order.pickupCode` and the shopkeeper typed it — which meant the shopkeeper alone could advance the order once they had heard six digits. Now the shop cannot move the order at all and the rider cannot move it without the shop's code, so a false handover needs both of them.

**What a code proves, stated precisely because "OTP at the door" oversells it.** That the holder chose to release the goods to whoever held this rider's session at that moment. It does *not* prove they were standing together: a shopkeeper can read six digits down a phone and a customer can text theirs to a rider stuck in traffic. It makes a handover need both parties, and `verifiedBy`/`verifiedAt` make a false one attributable. It is not proof of delivery.

Things that are easy to get wrong here:

- **Codes live in `OrderHandover`, never on `Order`, and `select: false` is not why they are safe.** Measured against this codebase, `select: false` on an embedded code hides it from a plain `find` — but `.select('handover')` returns every code under the parent, `.select('handover.delivery')` returns that one, and `aggregate()` ignores `select: false` entirely. On top of that `routes/rider.js` accept and deliver, and `routes/orders.js` claim, return a raw `order.toJSON()` straight to the rider. A code on the order is one careless projection away from exactly the wrong person; a code in its own collection cannot ride any order read of any shape. `db/migrations.js` → `migrateDroppedPickupCode()` unsets the old field, because a leftover `pickupCode` is a live value the rider has already seen.
- **A code is created the first time its holder opens it, and nowhere else.** Not at checkout, not when a stall claims, not on a status write. That gives one mint site instead of one per transition, needs no backfill for orders already in flight, and survives a market order being re-sourced to a different market (`sourcing.js`), which would have stranded a code minted eagerly at claim time. The unique `{ order, stage, stall }` index — `stall` null for everything but a market pickup, and null is indexed — is what makes two first reads racing insert one document. It still **fails closed**: a rider cannot redeem a code nobody has been shown (`CODE_NOT_ISSUED`).
- **A market order gets one pickup code per stall, not one per order.** A single code the rider heard at the first stall would tick off stalls they never walked to. An order only reaches `collecting` once every line is packed, so one collect always releases everything a stall holds and its code is then retired; a second collect is refused as `ALREADY_VERIFIED`, never waved through.
- **Attempts are counted before comparing, on the document, and five lock it.** The same conditional `$inc` shape as `services/otp.js`. A locked code is dead even with the right digits; only its holder can reissue. The per-order limiters in `middleware/rateLimit.js` add a time window on top, but `base.skip` switches every limiter off under test — the document cap is the bound a test can actually drive. One limiter bucket per stage (and per stall): fumbled pickup guesses must not spend the budget a rider needs at the customer's door.
- **The code is plaintext, deliberately.** A holder has to be shown it again after a reload or on a second device, and web storage is off limits for it, so a hash will not do. What hashing buys against the rider is bought by the holder filter, the attempt cap and a constant-time compare; what it would buy against a database dump is not bought. That trade is written on the model so it is not rediscovered as a bug.
- **Holder routes are scoped to the holder and nobody else — not even `developer`.** A route that can hand an operator one live code is a route that can hand them all of them. What `developer` gets instead is the override: `PATCH /api/orders/:id/status` still accepts `Out for Delivery` and `Delivered` from a developer, for the dead phone at the door, and records who used it in `statusHistory.by`. Every other role is refused with `PICKUP_CODE_REQUIRED` / `DELIVERY_CODE_REQUIRED`.
- **The customer's code is released only while the order is out for delivery.** Earlier, a rider could ask for it over the phone before collecting anything, and the customer would have nothing left to confirm the delivery with.
- **`OTPBoxGroup` takes `autoComplete`.** `one-time-code` is right for a code the user was *sent*, and the phone offers it from their messages. It is wrong for a code read off someone else's screen — a rider typing a shop's code would be offered their own sign-in code — so the entry passes `off`.
- **`busy` is on duty.** The server sets it the moment a rider accepts a job and refuses to take a busy rider off duty. `DeliveryPanel` used to read only `online`, so any reload mid-job showed "You are off duty" and hid the job — and with it the box to type the customer's code into, leaving the order uncompletable by anyone but a developer.

`test/handoverCodes.test.js` covers delivery, market stalls, the override and the leak checks; `test/shopPickupHandoff.test.js` covers the shop counter and was rewritten rather than extended, because it was the executable statement of the old direction. Leak assertions search for the code as a *whole value*, never with `JSON.stringify().includes` — six digits turn up inside phone numbers, timestamps and ObjectIds, and a leak check that fails at random teaches everyone to ignore it. `test/helpers.js` fetches codes through the holder's own route (`shopPickupCode`, `stallPickupCode`, `deliveryCode`), so a test that completes a handover also proves the holder could see the code.
...
**Every tool argument is attacker-influenced, because the customer's own message steers the tool call.** An id in a tool call is not a capability — scope the query to `user._id` exactly as a route handler would. `get_order_status` filtered `{ _id: orderId, user: user._id }` and `Order` has no `user` path (it is `customer`), so `strictQuery` dropped the clause and the tool reported **any** order in the database — number, status, payment status, total and date — to any signed-in customer who asked about an id. Nothing failed and nothing logged; see the `strictQuery` discussion under Server layout for why this whole class is silent.
...
**That applies inside aggregation pipelines too: sum paise, divide once.** `{ $sum: { $divide: ['$totalAmountPaise', 100] } }` turns every row into a float before adding it, so the error accumulates across every order the platform has ever taken; `{ $sum: '$totalAmountPaise' }` divided once at the response boundary is exact. `routes/markets.js` `/analytics` is the worked example — it sums `grossPaise`/`netPaise` and converts at the edge. `routes/developer.js` did the opposite in four places and was corrected.

**A rupee virtual does not survive `.lean()`.** `Product.price` and `Order.totalAmount` are virtuals over the paise fields, and `.lean()` returns plain documents with no virtuals — so `p.price` on a lean row is `undefined`, silently, and renders as a blank or a zero rather than throwing. Read the `*Paise` field and divide, or drop `.lean()`. `GET /api/developer/dump` shipped `price: undefined` and `total: undefined` for exactly this reason, in the one endpoint whose whole job is to show what is in the database.
...
**Platform commission has exactly one source of truth: `StallEarning.commissionPaise`.** `services/settlement.js` withholds it at the rate in `config.settlement.commissionBps` — which **defaults to zero** — on delivery, for market stalls and independent shops alike. Never re-derive it as a percentage of sales: the Developer Console reported `Math.round(allTimeSales * 0.1)`, so on any deployment that had not set `STALL_COMMISSION_BPS` the operator's dashboard showed substantial revenue the platform had not taken a paisa of. A figure from the ledger is also correctly *lower* than a share of gross, because commission is earned on delivery and not on placement.
...
- **`strictQuery` is on, so a misspelled field in a filter is silently dropped rather than failing.** `db/connect.js` sets it, and it is right to: it keeps a stray `$`-key or a client-supplied path from reaching the database. The cost is that `{ shopkeeper: id }` against a model whose field is `owner` does not error and does not match nothing — the path vanishes and what remains runs. A one-key filter becomes `{}` and matches **everything**.
  ...
  **A test that only asserts a 200 cannot see any of this, and `routes/developer.js` is the proof.** Every fault in that class needs a real row to surface: on an empty database a filter naming a field the schema does not have still returns `[]`, a `.populate()` of a non-path is never executed, and a status value outside the enum matches nothing whether it is spelled right or not. `test/developer.test.js` walked every endpoint asserting `200` and found none of eleven field-name faults, three more in `/dump`, and a `$match` on a status (`'Placed'`) that has never been an `ORDER_STATUSES` member. `test/developerConsoleData.test.js` is the companion that seeds the row first; put new assertions about that router there. `strictPopulate` is the one member of the family that *does* throw — but only once the query returns at least one document, which is the same reason.

- **A count in a heading must be a `countDocuments`, not the length of a capped page.** Reporting screens kept doing `find().limit(10)` and then putting `.length` in a title, so a triage panel quoted its own page size as the size of the problem: "10 Products Out of Stock" however many there were, and "₹X net flow across 100 recorded transactions" beside a figure aggregated over the whole ledger. Take the number from a count and the *names* from a small sample — they are two queries because they answer two questions. Where a payload really is a sample, say so in the key (`/developer/dump` returns `sampled`, not `counts`).
...
- **A named `manualChunks` entry is EAGER, whether or not anything eager imports it — this is the bundle trap, and it has cost real bytes twice.** `manualChunks` does not group modules, it *forces* them together; rolldown then duplicates a copy of react-dom into one of those vendor chunks for CJS interop, the eager app chunk imports react-dom from there, and the whole chunk gets a `<link rel="modulepreload">` in `dist/index.html`. A preload is a download at first paint, so the chunk is on the critical path no matter how lazily the app code reaches it.
  ...
  Measured: the unlucky chunk had become a 121 KB gzip `vendor` holding recharts and its d3 tree, so **every customer fetched the entire charting library to render a shop front with no chart on it** — first-load JS was ~228 KB gzip against the ~121 KB this file used to claim. Leaflet was one modal away from the same fate, and was in fact already being downloaded by every customer because `DeliveryLocationBar.jsx` imported `MapLocationPicker` statically.

  So the rule is: **anything large that only SOME screens need is left unassigned (`return undefined`) and made `lazy()` at every import site.** Unassigned hands placement back to the bundler, which puts a module only in the chunks that actually reach it — and it still emits a separate content-hashed chunk (`leaflet-*.js`, plus the recharts one), so the caching argument is unaffected; only the chunk's *name* changes. That pair of changes took first-load JS from ~228 KB gzip to ~130 KB.

  Currently unassigned and lazy at every call site: **Leaflet** (`MapLocationPicker`, `DeliveryRouteMap`) and **recharts** (`admin/views/DashboardOverview`, `UsageAnalytics`). If you add a rule to `manualChunks`, or a static import of one of those components, **check `dist/index.html` afterwards** — a name in that preload list is a download every customer pays for.

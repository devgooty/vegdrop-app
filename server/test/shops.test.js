...
/**
 * `createdBy` defaults to `owner`, matching what `POST /products` actually
 * stamps for a shopkeeper-created listing — the two are the same account in
 * every real row. Pass `createdBy` explicitly only to build a true legacy/
 * seeded row (both null), which is a different case: see
 * catalogOwnership.test.js for why that stays admin-only rather than
 * belonging to whichever shopkeeper reaches it first.
 */
async function seedProduct({ owner = null, createdBy = owner, name = 'Tomato', pricePaise = 9900 } = {}) {
...
test('a shopkeeper cannot edit or restock another shop product', async () => {
  ...
  assert.equal(edit.body.error.code, 'NOT_YOUR_PRODUCT');
...
/**
 * The shared/seeded platform catalog (no owner, no createdBy) is deliberately
 * excluded here — see catalogOwnership.test.js's "an unowned catalog row is
 * not editable by a vendor". Treating "unowned" as "first vendor to ask" would
 * hand the whole platform catalog to whoever reached it first; "mine" mirrors
 * that same boundary rather than promising a listing this account cannot
 * actually save changes to.
 */
test('mine returns only my own listings, never a competitor or the shared catalog', async () => {
  const shopA = await seedListedShop({ name: 'Shop A' });
  const shopB = await seedListedShop({ name: 'Shop B' });
  await seedProduct({ owner: shopA.user._id, name: 'A Tomato' });
  await seedProduct({ owner: shopB.user._id, name: 'B Potato' });
  ...
  assert.deepEqual(
    names,
    ['A Tomato'],
    'only rows this account actually created — and may actually edit — belong in "mine"'
  );
});
...
/**
 * The independent-shops feature does not reopen the hole self-registration
 * created. A vendor account is reachable by any stranger who clears a penny
 * drop (see routes/auth.js's vendor self-registration), so "the shared catalog
 * is common inventory" would mean any stranger who signs up can reprice or
 * empty stock on rows every shop is relying on. Only market_owner/developer
 * — accounts nobody self-registers into — may touch an unowned row.
 */
test('the shared catalog stays admin-only, even for a listed shop', async () => {
  const shop = await seedListedShop();
  const shared = await seedProduct({ name: 'Platform Onion' });
  ...
  const untouched = await Product.findById(shared._id).lean();
  assert.equal(untouched.stock, 500, 'refused, not partially applied');
});
